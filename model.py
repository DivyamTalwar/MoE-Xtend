...
class RotaryEmbedding(nn.Module):
    def __init__(
        ...
        device: Optional[torch.device] = None # Where to allocate the cos/sin tensors
    ) -> None:
...
class Cache:
    def __init__(self, batch_size, n_ctx, n_kv_heads, d_head, device: Optional[torch.device] = None):
    ...
    def snapshot(self):
        return (self.k.clone(), self.v.clone(), self.offset.clone())
...
class AttentionBlock(nn.Module):
    def __init__(self, configs: ModelConfigs, layer_idx: int = 0, device: Optional[torch.device] = None):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, configs: ModelConfigs, device: Optional[torch.device] = None):
        super().__init__()
        self.configs = configs
    ...
    @staticmethod
    def from_checkpoint(path: str, device: Union[str, torch.device] = "cuda") -> "Transformer":
